...
Na seção anterior, vimos que uma boa função hash deve tentar espalhar bem as chaves pelos buckets.
...
```text
1000 possíveis chaves
10 buckets
```

É impossível colocar todas as possíveis chaves em buckets diferentes.
...
Nesta aula, nos concentraremos apenas em **Separate Chaining**.

---

## 9. Encadeamento separado

### 9.1 Funcionamento

A ideia por trás do **tratamento de colisões por encadeamento** é simples.
...
Com Separate Chaining, isso não destrói a tabela.
...
<img src="./26_hash_table_hash_table_with_separate_chaining.png" width="800"/>
...
Além disso, assumimos que a posição escolhida para uma chave é independente da posição escolhida para as outras chaves.
...
Se mantivermos `α` pequeno, então as operações ficam próximas de:
...
```text
Busca esperada:    O(1)
Inserção esperada: O(1), se for necessário verificar duplicatas
Remoção esperada:  O(1)
```
...
```text
Busca no pior caso:    O(n)
Remoção no pior caso:  O(n)
Inserção no pior caso: O(n), se for necessário evitar duplicatas
```
...
Uma tabela hash busca oferecer:

```text
O(1)
```

em média ou em valor esperado, desde que duas condições sejam razoavelmente satisfeitas:
...
Na próxima aula, veremos como controlar esse segundo ponto usando **resizing** e **rehashing**.

### 9.4 Esboço de implementação em C++

Para implementar Separate Chaining, podemos criar uma tabela em que cada bucket é uma lista encadeada.

Primeiro, um nó simples:
...
Agora, uma classe `HashTable`:

```cpp
class HashTable {
private:
    Node** table;
    int capacity; // número de buckets, isto é, m
    int size;     // número de elementos armazenados, isto é, n
...
Esse código ainda não está completo.
...
remoção
destrutor
tratamento de chaves negativas
evitar elementos duplicados, se desejado
```

Mas ele já mostra a ideia principal:

```text
1. calcular o bucket;
2. acessar a lista daquele bucket;
3. inserir ou buscar dentro da lista.
```